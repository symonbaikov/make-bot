# 🔍 Чеклист для отладки бота

## Проблема: Бот не отвечает на команду /start

### Шаг 1: Проверьте webhook URL

В Railway для **Bot сервиса** (`@make-bot/bot`):

1. Откройте **Variables**
2. Проверьте `TELEGRAM_WEBHOOK_URL`:
   - Должен быть: `https://make-botbot-production.up.railway.app/webhook`
   - НЕ должен быть: `https://make-botbot-production.up.railway.app/` (без `/webhook`)

### Шаг 2: Проверьте логи бота

В Railway откройте **Bot сервис** → **Logs** и проверьте:

**Должно быть:**

```
Starting bot in webhook mode: https://make-botbot-production.up.railway.app/webhook
Webhook server listening on port 8080
Webhook set to: https://make-botbot-production.up.railway.app/webhook
```

**Если видите ошибки:**

- `Failed to set webhook` - проверьте URL
- `Error handling webhook update` - проверьте обработку команд
- Нет логов вообще - бот не запустился

### Шаг 3: Проверьте доступность webhook

Откройте в браузере:

```
https://make-botbot-production.up.railway.app/health
```

Должно вернуть:

```json
{ "status": "ok", "mode": "webhook" }
```

### Шаг 4: Проверьте переменные окружения

Убедитесь, что установлены:

- `TELEGRAM_BOT_TOKEN` - токен бота
- `TELEGRAM_WEBHOOK_URL` - полный URL с `/webhook`
- `API_URL` - URL бэкенда
- `NODE_ENV=production` - для продакшена

### Шаг 5: Проверьте обработку команды /start

После отправки `/start` в логах должно быть:

```
Start command received { userId: ..., username: ..., messageText: '/start' }
Start command { sessionId: 'none', userId: ... }
```

Если этих логов нет - команда не доходит до обработчика.

### Шаг 6: Временное решение - используйте polling

Если webhook не работает, можно временно использовать polling:

1. В Railway для Bot сервиса установите:

   ```
   ALLOW_POLLING_IN_PRODUCTION=true
   ```

2. Удалите или закомментируйте `TELEGRAM_WEBHOOK_URL`

3. Перезапустите бот

**Внимание:** Polling не рекомендуется для продакшена, используйте только для тестирования!

### Шаг 7: Проверьте, что бот запущен

В Railway проверьте:

- Статус сервиса должен быть "Running"
- Последний деплой должен быть успешным
- Нет ошибок в логах при запуске

## Быстрая диагностика

Выполните эти команды для проверки:

```bash
# Проверка health endpoint
curl https://make-botbot-production.up.railway.app/health

# Проверка webhook endpoint (должен вернуть ошибку, это нормально)
curl -X POST https://make-botbot-production.up.railway.app/webhook
```

## Частые проблемы

1. **Webhook URL неправильный** - должен заканчиваться на `/webhook`
2. **Бот не запущен** - проверьте статус в Railway
3. **Ошибки в коде** - проверьте логи на наличие ошибок
4. **Telegram не может достучаться** - проверьте доступность домена
