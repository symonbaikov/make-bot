...
export function Dashboard() {
  ...
  const statsCards = [
    ...
    { label: 'Загальний дохід', value: `$${stats.totalRevenue.toFixed(2)}`, icon: DollarSign, color: 'text-green-400' },
    { label: 'Конверсія', value: `${conversionRate}%`, icon: TrendingUp, color: 'text-purple-400' },
    { label: 'Завершено', value: stats.conversionFunnel.completed, icon: CheckCircle, color: 'text-teal-400' },
  ];
          ...
          <GlassCard className="p-6 h-full">
            ...
            <div className="space-y-6">
              {[
                ...
                { label: 'Очікування оплати', value: stats.conversionFunnel.awaitingPayment, color: 'bg-yellow-500' },
                { label: 'Оплачено', value: stats.conversionFunnel.paid, color: 'bg-green-500' },
                { label: 'Завершено', value: stats.conversionFunnel.completed, color: 'bg-teal-500' },
              ].map((item, index) => (
                <div key={item.label}>
                  ...
                  <div className="w-full bg-white/5 rounded-full h-2 overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${stats.conversionFunnel.started > 0 ? (item.value / stats.conversionFunnel.started) * 100 : 0}%` }}
          ...
          <GlassCard className="p-6 h-full">
            ...
            <div className="space-y-4">
              {stats.revenueByPlan.map((item, index) => (
                ...
                >
                  <span className="text-sm font-medium text-gray-300">{item.plan}</span>
                  <div className="flex items-center space-x-4">
                    <span className="text-sm text-gray-400">{item.count} продажів</span>
                    <span className="text-sm font-bold text-green-400">${item.revenue.toFixed(2)}</span>
                  </div>
                </motion.div>
              ))}
            </div>
